...
constexpr std::size_t encoded_size(std::size_t n) noexcept { return 4 * ((n + 2) / 3); }

/// Returns max bytes needed to decode a base64 string
constexpr std::size_t decoded_size(std::size_t n) noexcept { return ((n & 3) == 0) ? (n / 4 * 3) : (3 * n / 4); }
...
template <std::size_t N, std::size_t M = encoded_size(N)> constexpr auto encode(const std::array<char, N>& src) noexcept {
...
template <std::size_t N, std::size_t M = encoded_size(N)> std::string encode_str(const std::array<char, N>& src) noexcept {
    std::string ret{};
    ret.resize(M);
...
template <std::size_t N, std::size_t M = decoded_size(N)> constexpr auto decode(const std::array<char, N> src) noexcept {
    std::array<unsigned char, M> ret{};
    auto res = decode(ret.data(), src.data(), N);
    return ret;
}

template <std::size_t N, std::size_t M = decoded_size(N)> constexpr auto decode(std::string_view src) noexcept {
    struct {
